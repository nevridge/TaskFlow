...
{
    private readonly Mock<IJournalEntryService> _service = new();
    ...
    {
        _controller = new JournalEntriesController(_service.Object, _validator.Object);
    }
    ...
    public async Task GetAll_ShouldReturnOk()
    {
        ...
        _service.Setup(s => s.GetAllAsync()).ReturnsAsync(entries);
    ...
    {
        _service.Setup(s => s.GetByIdAsync(123)).ReturnsAsync((JournalEntry?)null);
    ...
    public async Task Create_ShouldReturnConflict_WhenDateAlreadyExists()
    {
        ...
        _service.Setup(s => s.GetByDateAsync(date))
        ...
        _service.Verify(s => s.CreateAsync(It.IsAny<JournalEntry>()), Times.Never);
    }
    ...
    {
        var date = new DateOnly(2026, 5, 10);
        _service.Setup(s => s.GetByDateAsync(date)).ReturnsAsync((JournalEntry?)null);
    ...
    public async Task Create_ShouldReturnCreated_WhenValid()
    {
        ...
        _service.Setup(s => s.CreateAsync(It.IsAny<JournalEntry>()))
    ...
    public async Task Create_ShouldReturnConflict_WhenUniqueRaceOccurs()
    {
        ...
        _service.SetupSequence(s => s.GetByDateAsync(date))
        ...
        _service.Setup(s => s.CreateAsync(It.IsAny<JournalEntry>()))
            .ThrowsAsync(new DbUpdateException("constraint", new Exception("UNIQUE constraint failed: JournalEntries.Date")));
    ...
    {
        _service.Setup(s => s.GetByIdAsync(44)).ReturnsAsync((JournalEntry?)null);
    ...
    {
        _service.Setup(s => s.GetByIdAsync(7)).ReturnsAsync(new JournalEntry { Id = 7, Title = "a", Date = new DateOnly(2026, 5, 10) });

        var result = await _controller.Delete(7);

        result.Should().BeOfType<NoContentResult>();
        _service.Verify(s => s.DeleteAsync(7), Times.Once);
    }
}
