...
public class JournalEntryService(IJournalEntryRepository repo) : IJournalEntryService
{
    ...
    public async Task<IEnumerable<TaskItem>> GetTodosAsync(int entryId) => await _repo.GetTodosAsync(entryId);
    public async Task<bool> TodoExistsAsync(int entryId, int taskItemId) => await _repo.TodoExistsAsync(entryId, taskItemId);
    public async Task AddTodoAsync(int entryId, int taskItemId) => await _repo.AddTodoAsync(entryId, taskItemId);
    public async Task RemoveTodoAsync(int entryId, int taskItemId) => await _repo.RemoveTodoAsync(entryId, taskItemId);
}
